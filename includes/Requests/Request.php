...
class Request implements FormRequestInterface {
    ...
    public function validate() {
        if ( ! isset( $this->data['_wpnonce'] ) || ! wp_verify_nonce( $this->data['_wpnonce'], $this->get_nonce() ) ) {
            $this->addError( 'nonce', __( 'Nonce verification failed', 'pay_check_mate' ) );

            wp_die( __( 'Nonce verification failed', 'pay_check_mate' ) );
			// throw new Exception( __( 'Nonce verification failed', 'pay_check_mate' ) );
        }
        ...
        if ( ! empty( $fillable ) ) {
            foreach ( $fillable as $item ) {
                if ( ! array_key_exists( $item, $this->data ) ) {
                    $this->addError( $item, $item . __( ' key is required for this form request.', 'pay_check_mate' ) );
                }

                // Check if this key value is empty.
                if ( '' === $this->data[ $item ] && empty( $this->data[ $item ] ) ) {
                    $this->addError( $item, $item . __( ' key can not be empty.', 'pay_check_mate' ) );
                }
            }
        }
    }
    ...
    public static function get_nonce(): string {
        if ( empty( static::$nonce ) ) {
            throw new Exception( esc_html__( 'Nonce is not defined for this form request.', 'pay_check_mate' ) );
        }
    ...
    public static function get_rules(): array {
        if ( empty( static::$rules ) ) {
            throw new Exception( esc_html__( 'Make sure you have defined rules for this form request.', 'pay_check_mate' ) );
        }
    ...
    public static function get_fillable(): array {
        if ( empty( static::$fillable ) ) {
            throw new Exception( esc_html__( 'Fillable are not defined for this form request.', 'pay_check_mate' ) );
        }
