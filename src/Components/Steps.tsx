...
export const Steps = ({steps, currentStep, setStep}: StepPropsType) => {
    return (
        <ol className="flex bg-gray-200 justify-between items-center w-full p-3 space-x-2 text-sm font-medium text-center text-gray-500 shadow-sm sm:text-base sm:p-4 sm:space-x-4">
            {steps.map((step, index) => (
                <li onClick={() => setStep(index + 1)} key={index} className={`cursor-pointer flex items-center ${index < currentStep -1 ? 'text-green-600' : index < currentStep ? 'text-blue-600' : ''}`}>
